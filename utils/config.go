...
// GetBoolWithDefault returns a config bool, falling back to `def` if the
// key isn't set. Useful for feature flags like
// `system.registration.require_code`.
...
// GetStringWithDefault returns a config string, falling back to `def`.

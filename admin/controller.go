...
// UsernameMigrationForm — admin-side username change.
// Used by /admin/user/username to let an administrator rename any user.
type UsernameMigrationForm struct {
        Id       int64  `json:"id"`
...
// UpdateUsernameAPI — admin renames a user.
//
// POST /admin/user/username
// Body: { "id": <int>, "username": "<new name>" }
//
// Validates the new username (length 2..24, unique) and clears the
// user's auth cache so subsequent JWT validations pick up the change.
func UpdateUsernameAPI(c *gin.Context) {
        db := utils.GetDBFromContext(c)
        cache := utils.GetCacheFromContext(c)

        var form UsernameMigrationForm
        if err := c.ShouldBindJSON(&form); err != nil {
                c.JSON(http.StatusOK, gin.H{
                        "status":  false,
                        "message": err.Error(),
        ...
        if err := usernameMigration(db, cache, form.Id, form.Username); err != nil {
                c.JSON(http.StatusOK, gin.H{
                        "status":  false,
                        "message": err.Error(),
                })

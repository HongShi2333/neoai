...
# [🥳 NeoAI](https://neoai.dev)

#### 🚀 次世代AIGCワンストップビジネスソリューション

#### *"NeoAI > [Next Web](https://github.com/ChatGPTNextWeb/ChatGPT-Next-Web) + [One API](https://github.com/songquanpeng/one-api)"*


English · [简体中文](./README_zh-CN.md) · [日本語](./README_ja-JP.md) · [ドキュメント](https://neoai.dev) · [Discord](https://discord.gg/rpzNSmqaF2) · [デプロイガイド](https://neoai.dev/docs/deploy)

[![NeoAI: #1 Repo Of The Day](https://trendshift.io/api/badge/repositories/6369)](https://trendshift.io/repositories/6369)
...
4. 👀 **マルチテーマサポート**: 複数のテーマ切り替えをサポート、ライトテーマの**ライトモード**とダークテーマの**ダークモード**を含む。 👉 [カスタムカラースキーム](https://github.com/neoai-dev/neoai/blob/main/app/src/assets/globals.less)
5. 📚 **国際化サポート**: 国際化をサポートし、複数の言語切り替えをサポート 🇨🇳 🇺🇸 🇯🇵 🇷🇺 👉 翻訳の貢献を歓迎します [Pull Request](https://github.com/neoai-dev/neoai/pulls)
...
9. 📖 **豊富なファイル解析**: **すぐに使える**、**すべてのモデル**のファイル解析をサポート (PDF / Docx / Pptx / Excel / 画像形式の解析)、**より多くのクラウド画像ストレージソリューション** (S3 / R2 / MinIO など) をサポート、**OCR画像認識**をサポート 👉 詳細はプロジェクト [NeoAI Blob Service](https://github.com/coaidev/blob-service) を参照 (Vercel / Dockerのワンクリックデプロイをサポート)
...
> - ...
>
> [👉 詳細はこちら](https://www.neoai.dev/docs/contact)
...
## 📦 デプロイ
> [!TIP]
> **デプロイが成功した後、管理者アカウントは `root` で、デフォルトのパスワードは `neoai123456` です**
...
> 2. デプロイが完了したら、ドメイン名にアクセスし、ユーザー名 `root` とパスワード `neoai123456` を使用してバックエンド管理にログインします。チャットニオのバックエンドでパスワードを変更するように指示に従ってください。
...
```shell
git clone --depth=1 --branch=main --single-branch https://github.com/neoai-dev/neoai.git
cd neoai
...
> 
> 安定版を使用するには、`programzmh/neoai:stable` を `programzmh/neoai:latest` の代わりに使用してください
...
docker run -d --name neoai \
   ...
   -e MYSQL_DB=neoai \
   -e MYSQL_USER=root \
   -e MYSQL_PASSWORD=neoai123456 \
   -e REDIS_HOST=localhost \
   -e REDIS_PORT=6379 \
   -e SECRET=secret \
   -e SERVE_STATIC=true \
   programzmh/neoai:latest
```
...
docker stop neoai
docker rm neoai
docker pull programzmh/neoai:latest
```
...
```shell
git clone https://github.com/neoai-dev/neoai.git
cd neoai
...
nohup ./neoai > output.log & # バックグラウンドで実行するためにnohupを使用
```
...
そのため、**NeoAI** が誕生しました。強力なAPI分散システムと豊富なユーザーインターフェースデザインを持つプロジェクトを作成し、次世代のオープンソースAIGCプロジェクトのワンストップ商用ソリューションになることを目指しています。

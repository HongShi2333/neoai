...
func AuthMiddleware() gin.HandlerFunc {
        return func(c *gin.Context) {
                ...
                // NeoAI: API routes are ALWAYS mounted under `/api` now (see
                // main.go registerApiRouter). Strip the prefix before doing
                // the admin-gate check so the path comparison below works in
                // both serve_static modes and split deployments.

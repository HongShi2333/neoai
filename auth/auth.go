...
func SignUp(c *gin.Context, form RegisterForm) (string, error) {
        ...
        // NeoAI: registration code gate. If enabled, the user must supply a
        // valid unused code. Consume it atomically BEFORE creating the user
        // so a failed registration doesn't burn a code use.
        bonusQuota := float32(0)
        if IsRegistrationCodeRequired() {
                bonus, err := ConsumeRegistrationCode(db, form.RegistrationCode)
                if err != nil {
                        return "", err
                }
                bonusQuota = bonus
        ...
        // grant any bonus quota attached to the consumed registration code
        if bonusQuota > 0 {
                user.IncreaseQuota(db, bonusQuota)
        }
...
func (u *User) Validate(c *gin.Context) bool {
        ...
        // NeoAI: cache is optional. If it's nil or unreachable, skip the
        // cache lookup and fall through to the DB check. Don't fail auth

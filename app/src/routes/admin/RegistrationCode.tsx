...
function RegistrationCodePage() {
    ...
    if (resp.status) {
      toast.success(
        value
          ...
          : t("admin.reg-code.disabled") || "Registration code disabled",
      );
    } else {
  ...
  async function copyCode(code: string) {
    ...
    toast.success(t("admin.reg-code.copied") || "Code copied");
  }
      ...
      <Card>
        <CardHeader className="flex flex-row items-center">
          <CardTitle className="mr-auto">
            {t("admin.reg-code.title") || "Registration Codes"}
          </CardTitle>
          <Button size="sm" onClick={() => setGenOpen(true)}>
            <Plus className="h-4 w-4 mr-1.5" />
            {t("admin.reg-code.generate") || "Generate"}
          </Button>
        </CardHeader>
        <CardContent>
          {/* Toggle: require registration code at signup */}
          <div className="flex items-center gap-3 mb-4 pb-4 border-b">
            <Switch checked={required} onCheckedChange={toggleRequired} />
            <div className="flex-1">
              <Label className="font-medium">
                {t("admin.reg-code.require-label") || "Require registration code"}
              </Label>
              <p className="text-xs text-muted-foreground mt-0.5">
                {t("admin.reg-code.require-desc") ||
                  "When enabled, new users must supply a valid registration code during signup. Existing users are not affected."}
              </p>
            </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p>{t("admin.reg-code.no-codes") || "No codes yet"}</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t("admin.reg-code.code") || "Code"}</TableHead>
                  <TableHead className="w-24">
                    {t("admin.reg-code.uses") || "Uses"}
                  </TableHead>
                  <TableHead className="w-24">
                    {t("admin.reg-code.quota") || "Bonus quota"}
                  </TableHead>
                  <TableHead>{t("admin.reg-code.note") || "Note"}</TableHead>
                  <TableHead className="w-24">
                    {t("admin.reg-code.status") || "Status"}
                  </TableHead>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          {!c.disabled && (
                            <DropdownMenuItem onClick={() => disable(c.id)}>
                              ...
                              {t("admin.reg-code.disable") || "Disable"}
                            </DropdownMenuItem>
                          )}
                          <DropdownMenuItem
                            onClick={() => remove(c.id)}
                          ...
                          >
                            ...
                            {t("admin.reg-code.delete") || "Delete"}
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
...
function GenerateDialog(props: {
  ...
  async function submit() {
    ...
    if (resp.status && resp.data) {
      toast.success(
        `${resp.data.length} codes generated. Click any code to copy.`,
      );
      onGenerated();
    } else {
      toast.error(resp.error || "Failed");
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {t("admin.reg-code.generate-title") || "Generate registration codes"}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <div>
            <Label>{t("admin.reg-code.number") || "Number of codes"}</Label>
            <NumberInput
              value={number}
              min={1}
              max={1000}
          ...
          <div>
            <Label>
              {t("admin.reg-code.max-uses") || "Max uses per code"}
            </Label>
            <NumberInput
              value={maxUses}
              min={1}
            ...
            <p className="text-xs text-muted-foreground mt-1">
              How many distinct users can register with each code.
            </p>
          </div>
          <div>
            <Label>
              {t("admin.reg-code.bonus-quota") || "Bonus quota (optional)"}
            </Label>
            <NumberInput
              value={quota}
              min={0}
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">
              Quota granted to each new user who registers with this code. 0 = no bonus.
            </p>
          </div>
          <div>
            <Label>{t("admin.reg-code.note") || "Note (optional)"}</Label>
            <Input
              value={note}
              onChange={(e) => setNote(e.target.value)}
              placeholder="e.g. distributed on Discord"
            />
          </div>
        </div>
        <DialogFooter>

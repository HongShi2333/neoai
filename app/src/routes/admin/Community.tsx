...
// Different from the user-facing /community route, this page lives in
// the admin console and lets administrators:
//   - list ALL channels (even those they can't personally "see" via
//     visibility rules, because admins bypass visibility)
//   - create / edit / delete channels
//   - configure visibility + send permissions
//
// The user-facing /community route is the actual chat UI.
...
function Community() {
  ...
  async function refresh() {
    setLoading(true);
    // Use the admin-scoped endpoint so we see every channel regardless
    // of visibility rules — admins bypass visibility in the backend.
  ...
  return (
    <div className={`community-admin`}>
      <Card className={`admin-card community-card`}>
        <CardHeader className={`select-none flex flex-row items-center`}>
          <CardTitle className="mr-auto">{t("admin.community.title")}</CardTitle>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t("admin.community.send-permission")}</TableHead>
          ...
          {visibility === "roles" && (
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Comma-separated group names: normal, basic, standard, pro, admin
              </p>
            </div>
          )}
